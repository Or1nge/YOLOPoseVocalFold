...
from yoloposevf.geometry import ImageSize, clip_bbox, keypoint_bbox, union_bbox
...
class AnnotationConfig:
    bbox_label: str = "vocal_fold_roi"
    point_labels: tuple[str, str, str, str] = ("kp1", "kp2", "kp3", "kp4")
    class_id: int = 0
    class_name: str = "vocal_fold_roi"
    visibility: int = 2
    correction_margin_x: float = 0.18
    correction_margin_y: float = 0.18


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert LabelMe ROI-box plus 4-keypoint annotations to YOLO-Pose format."
    )
...
def extract_bbox(shapes: list[dict[str, Any]], cfg: AnnotationConfig) -> tuple[float, float, float, float]:
...
def split_items(items: list[dict[str, Any]], val_ratio: float, test_ratio: float, seed: int) -> dict[str, list[dict[str, Any]]]:
...
def convert(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for json_path in sorted(args.labelme_dir.glob("*.json")):
        try:
            ...
            bbox = corrected_bbox(extract_bbox(shapes, cfg), keypoints, image_size, cfg)
    ...
    splits = split_items(items, args.val_ratio, args.test_ratio, args.seed)
