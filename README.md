...
`confidence_curve` 会同时作用于检测框置信度和关键点置信度；当前 `tanh` 曲线在 0.65 附近拉开中低置信度和高置信度，同时避免简单平方把 0.9 这类高置信度也压低太多。旧行为可用 `confidence_curve=power` 且 `confidence_gamma=2.0` 复现。ROI 面积和关键点图像边界 gate 用于降低明显非声带区域的“可用框”风险。ROI 相对面积的分母使用裁掉黑边后的有效图像面积；V1.1 blackpad 推理会先在原图中找非黑边前景区域，再把该区域映射回 padded 坐标系。
